...
<body>
<div id="new">
<hr><center><h1>Bug Detail Edit Form</h1></center><hr>
	<% 
...
int f=bean.getBug_type_id();
		%>
	<form action="Bug_DetailsUpdateServlet" name="bug_details" id="bug_details"
		method="post">
		<input type="hidden" name="bug_id" value=<%=bean.getBug_id()%>>
		<table align="center">
			<tr>
				<td>Task Name</td>
								<%{
				...
				int test=bean_b.getTask_id();%>
						<option value="<%=test%>" <%if(a ==test) {%> selected="selected" <%}%>><%=bean_b.getProject_name()%></option>
				<%}} }%>
			...
			<tr>
				<td>Reported Date</td>
				<td><input type="text" id="reported_date" name="reported_date" class="validate[required] text-input datepicker"
