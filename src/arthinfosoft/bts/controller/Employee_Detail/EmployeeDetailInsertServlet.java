...
public class EmployeeDetailInsertServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (email.length() > 30) {
			request.setAttribute("email_length",
		...
		if (!m.find()) {
			request.setAttribute("email_match",
					"<font color=red>*It is not a valid email.</font>");
			isError = true;
		...
		if (password.length() > 20) {
			request.setAttribute("password_length",
		...
		if (security_question.length() > 50) {
			request.setAttribute("security_question_length",
		...
		if (answer.length() > 20) {
			request.setAttribute("answer_length",
					"<font color=red>*cannot be greater than 20 characters.</font>");
			isError = true;
		...
		if (employee_firstname.length() > 20) {
			request.setAttribute("employee_firstname_length",
					"<font color=red>*cannot be greater than 20 characters.</font>");
			isError = true;
		...
		if (employee_lastname.length() > 20) {
			request.setAttribute("employee_lastname_length",
					"<font color=red>*cannot be greater than 20 characters.</font>");
			isError = true;
		...
		if (permanent_address.length() > 100) {
			request.setAttribute("permanent_address_length",
		...
		if (temporary_address.length() > 100) {
			request.setAttribute("temporary_address_length",
					"<font color=red>*cannot be greater than 100 characters.</font>");
			isError = true;
		...
		if (mobile.length() > 14) {
			request.setAttribute("mobile_length",
		...
		if (landline.length() > 13) {
			request.setAttribute("landline_length",
		...
		if (isError) {
			...
			request.getRequestDispatcher("EmployeeDetailInsertPage.jsp")
			...
			if (flag) {
				...
				response.sendRedirect("EmployeeDetailInsertPage.jsp");
			}
		}
